# app/services/pubmed_literature.py
...
# -------------------------------------------------
# SUMMARIES (CORRECT DOCSUM PARSER)
# -------------------------------------------------
...
def fetch_pubmed_summaries(pmids: List[str]) -> List[Dict]:
    ...
    # -------------------------------------------------
    # DOCSUM PARSER (REAL PUBMED FORMAT)
    # -------------------------------------------------

    for doc in root.findall(".//DocSum"):
        # PMID is a DIRECT child, not an Item
        ...
        # 🔎 PER-STUDY DEBUG (safe & useful)
        logger.debug(
            f"Parsed PMID={pmid} | year={year} | "
            f"pubtypes={article_types} | title={title[:80]}"
        )
...
def fetch_pubmed_abstracts(pmids: List[str]) -> Dict[str, str]:
    """
    Fetch abstracts in small chunks to avoid PubMed efetch timeouts.
    Fails gracefully and never raises upstream.
    """
    if not pmids:
        return {}

    abstracts: Dict[str, str] = {}
    BATCH_SIZE = 5  # safe limit
    ...
    for i in range(0, len(pmids), BATCH_SIZE):
        ...
        try:
            ...
            logging.getLogger("pubmed-service").warning(
                f"⚠️ efetch failed for PMIDs {batch}: {e}"
            )
            continue
